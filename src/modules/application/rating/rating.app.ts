...
import { RatingDto } from "../../contracts/rating/rating.dto"
...
class RatingAppService extends ApplicationService {
  ...
  async insertRating(
    ratingInsert: IRatingInsert,
  ): Promise<Response<RatingDto>> {
    const response = new ResponseManager<RatingDto>()

    try {
      await this.userManager.get(ratingInsert.userId)
      const trip = await this.tripManager.get(ratingInsert.tripId)
      const isFinished = trip.tripState.some(
        (element) => element.isCurrent && element.state === TripState.Finished,
      )
      if (!isFinished) {
        throw new ServiceException(
          LocalizeError.getErrorByCode(TripErrorCodes.TripNotFinished),
        )
      }
