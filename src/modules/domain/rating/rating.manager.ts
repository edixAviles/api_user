...
class RatingManager {
  ...
  async getRatingByUserAndTrip(
    ...
    const rating = await this.ratingRepository.getRatingByUserAndTrip(
      userId,
      tripId,
    )
    return rating
  ...
  async insert(ratingInsert: IRatingInsert): Promise<Rating> {
    const ratingFound = await this.getRatingByUserAndTrip(
      ratingInsert.userId,
      ratingInsert.tripId,
    )
    if (ratingFound) {
      throw new ServiceException(
        LocalizeError.getErrorByCode(RatingErrorCodes.TripIsAlreadyRated),
      )
    }
  ...
  private foundEntity = async (id: ObjectId): Promise<Rating> => {
    ...
    if (!rating) {
      ...
      const error = LocalizeError.getErrorByCode(
        RatingErrorCodes.EntityNotFound,
        errorParams,
      )
