...
class UserManager {
  ...
  async update(userUpdate: IUserUpdate): Promise<User> {
    ...
    user.email = {
      ...
      isVerified:
        userUpdate.email === entity.email.data
          ? entity.email.isVerified
          : false,
    }
    user.cellPhone = {
      data: userUpdate.cellPhone,
      isVerified:
        userUpdate.cellPhone === entity.cellPhone.data
          ? entity.cellPhone.isVerified
          : false,
    }
  ...
  async beDriver(userUpdate: IUserUpdateToDriver): Promise<User> {
    ...
    if (entity.isDriver) {
      const error = LocalizeError.getErrorByCode(
        UserErrorCodes.IsAlreadyDriver,
      )
  ...
  async updateLicencePhoto(
    userUpdate: IDriverUpdateLicencePhoto,
  ): Promise<User> {
    const entity = await this.foundEntity(userUpdate.id)
  ...
  async updatePoliceRecord(
    userUpdate: IDriverUpdatePoliceRecord,
  ): Promise<User> {
    const entity = await this.foundEntity(userUpdate.id)
  ...
  async updatePassword(userUpdate: IUserUpdatePassword): Promise<void> {
    ...
    const currentEncryptedPassword = CryptoJS.SHA256(
      userUpdate.currentPassword,
    ).toString()
    if (entity.password != currentEncryptedPassword) {
      const error = LocalizeError.getErrorByCode(
        UserErrorCodes.IncorrectCurrentPassword,
      )
      throw new ServiceException(error)
    }

    const newEncryptedPassword = CryptoJS.SHA256(
