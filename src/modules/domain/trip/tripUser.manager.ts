...
class TripUserManager {
  ...
  async getTripsUserByState(
    ...
    const tripsUser = await this.tripUserRepository.getTripsUserByState(
      tripId,
      state,
    )
    return tripsUser
  ...
  async getTripsUserByUser(
    ...
    const tripsUser = await this.tripUserRepository.getTripsUserByUser(
      userId,
      state,
    )
    return tripsUser
  ...
  async pickUpPassenger(id: ObjectId): Promise<void> {
    ...
    const tripUser = new TripUser()
    tripUser._id = id
    tripUser.tripState = this.getNewState(
      tripUserFound.tripState,
      TripUserState.OnTheWayToYou,
    )
  ...
  async startTrip(id: ObjectId): Promise<void> {
    const tripUserFound = await this.validateTrip(id, [
      TripUserState.OnTheWayToYou,
    ])
  ...
  async cancel(tripCancel: ITripUserCancel): Promise<void> {
    const tripUserFound = await this.validateTrip(tripCancel.id, [
  ...
  private validateTrip = async (
    ...
    if (!isAvailable) {
      ...
      const error = LocalizeError.getErrorByCode(
        TripUserErrorCodes.NotAvailable,
        errorParams,
      )
  ...
  private getNewState = (
    tripUserStates: DataTripUserStates[],
    state: string,
    observation?: string,
  ) => {
  ...
  private foundEntity = async (id: ObjectId): Promise<TripUser> => {
    ...
    if (!entity) {
      ...
      const error = LocalizeError.getErrorByCode(
        TripUserErrorCodes.EntityNotFound,
        errorParams,
      )
      throw new ServiceException(error)
    }
